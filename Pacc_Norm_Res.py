...
def run_this():
        ...
        while t[-1] < tend:
            ...
            lr = max(2/(1+exp(-2*(current_strat1+current_strat2))),lamn) #lamn
            cr = cn #max(0.8/(1+exp(-2*(current_strat1+current_strat2))),cn) #cn

            rates = [current_n*r, current_n*(m1/(lamn+b*current_strat1)+m2/(lamn+b*current_strat2)+r*((current_pacc+current_n+current_res)/K)),current_n*(cn*m1/(lamn+b*current_strat1)+cn*m2/(lamn+b*current_strat2)+g),
                     current_pacc*d,current_pacc*a,
                     current_res*r,current_res*(m1/(lr+b*current_strat1)+m2/(lr+b*current_strat2)+r*((current_pacc+current_n+current_res)/K)),current_res*(cr*m1/(lr+b*current_strat1)+cr*m2/(lr+b*current_strat2)+g),current_res*mu]
            ...
            #PACC switch event
                    ...
                    if random.uniform(0,1)>mut: #using same mutation rate, but larger breadth
                        ...
                        if m1==dose: #change if giving concurrently
                            strat2.append(strat2[-1])
                            new_strat = strat1[-1]+np.random.normal(0,0.05)
                            if drug1(new_strat,m1)<drug1(strat1[-1],m1):
                                    strat1.append(new_strat)
                                    res.append(res[-1] + 2)
                            elif drug1(new_strat,m1)==drug1(strat1[-1],m1):
                            ...
                            strat1.append(strat1[-1])
                            new_strat = strat2[-1]+np.random.normal(0,0.05)
                            if drug2(new_strat,m2)<drug2(strat2[-1],m2):
                                    strat2.append(new_strat)
                                    res.append(res[-1] + 2)
                            elif drug2(new_strat,m2)==drug2(strat2[-1],m2):
